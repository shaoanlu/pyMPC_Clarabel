...
class MPCController:
    ...
    def __init__(
        ...
        # QP problem instance
        if qp_backend.lower() not in ["osqp", "proxqp", "clarabel"]:
            raise ValueError(f"Received unknown QP backend {qp_backend}. Expect either 'osqp' or 'proxqp'.")
    ...
    def setup(self, solve=True):
        ...
        elif self.qp_backend.lower() == "clarabel":
    ...
    def output(
        ...
        elif self.qp_backend.lower() == "clarabel" and self.res.status == clarabel.SolverStatus.Solved:
            uMPC = self.res.x[(Np + 1) * nx : (Np + 1) * nx + nu]
        else:
            uMPC = self.u_failure
    ...
    def solve(self):
        ...
        if self.qp_backend.lower() == "osqp":
            ...
            self.prob.settings.initial_guess = proxsuite.proxqp.InitialGuess.WARM_START_WITH_PREVIOUS_RESULT
            self.prob.solve()
            self.res = self.prob.results
        elif self.qp_backend.lower() == "clarabel":
            res = self.prob.solve()
            self.res = ClarabelSolution(x=np.array(res.x), obj_val=res.obj_val, status=res.status)
        ...
        if self.qp_backend.lower() == "osqp":
            ...
            if self.res.status == clarabel.SolverStatus.PrimalInfeasible:
                warnings.warn("Clarabel did not solve the problem!")
                if self.raise_error:
    ...
    def _update_QP_matrices_(self):
        ...
        self.l[(Np + 1) * nx + (Np + 1) * nx + (Nc) * nu : (Np + 1) * nx + (Np + 1) * nx + (Nc) * nu + nu] = (
            Dumin + uminus1_rh[0:nu]
        )  # update constraint on \Delta u0: Dumin <= u0 - u_{-1}
        self.u[(Np + 1) * nx + (Np + 1) * nx + (Nc) * nu : (Np + 1) * nx + (Np + 1) * nx + (Nc) * nu + nu] = (
        ...
        if self.JX_ON:
            if xref.ndim == 2 and xref.shape[0] >= Np + 1:  # xref is a vector per time-instant! experimental feature
                ...
                q_X += np.hstack([np.kron(np.ones(Np), -Qx.dot(xref)), -QxN.dot(xref)])  # x0... x_N-1  # x_N
                if self.COMPUTE_J_CNST:
    ...
    def _compute_QP_matrices_(self):
        ...
        if self.Nc < self.Np:  # expand A matrix if Nc < Nu (see notes)
            iBu = sparse.vstack([iBu, sparse.hstack([sparse.csc_matrix((Np - Nc, Nc - 1)), np.ones((Np - Nc, 1))])])
